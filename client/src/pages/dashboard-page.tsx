...
const periodStart = (period: Period) => {
  ...
  if (period === "ytd") return isoDate(new Date(today.getFullYear(), 0, 1));
  const months = period === "3m" ? 2 : period === "6m" ? 5 : 11;
  return isoDate(new Date(today.getFullYear(), today.getMonth() - months, 1));
};
...
export function DashboardPage() {
  ...
  const filters = useMemo(() => ({ ...(branchId ? { branchId } : {}), from: periodStart(period), to: isoDate(new Date()) }), [branchId, period]);
