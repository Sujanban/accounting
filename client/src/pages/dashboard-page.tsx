...
export function DashboardPage() {
  ...
  const activeAssets = fixedAssets.data?.filter((asset) => asset.status === "ACTIVE") ?? [];
  const activeAssetValue = activeAssets.reduce((total, asset) => total + asset.purchaseValue, 0);
  const pendingLeaveRequests = leaveRequests.data?.filter((request) => request.status === "PENDING").length;
  ...
  return (
    <Flex direction="column" gap="5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <Card size="3"><Text color="gray" size="2">Active employees</Text><Heading mt="2" size="6">{employees.data?.length ?? "—"}</Heading><Text as="p" mt="2" size="2" color="gray"><Link to="/payroll">{pendingLeaveRequests ?? "—"} pending leave request{pendingLeaveRequests === 1 ? "" : "s"}</Link></Text></Card>
      </div>
    </Flex>
  );
}
