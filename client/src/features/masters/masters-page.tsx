...
function CatalogEditPage({ type, item, categories, contactGroups, branches }: { type: CatalogResource; item: EditableCatalogRecord; categories: { id: string; name: string; isActive: boolean }[]; contactGroups: { id: string; name: string; isActive: boolean }[]; branches: { id: string; name: string; isActive: boolean }[] }) {
  const navigate = useNavigate();
  ...
  const fields = () => {
    ...
    if (type === "warehouses") return <><label>Branch<RequiredMark /><AppSelect name="branchId" defaultValue={value("branchId")} required><option value="">Select branch</option>{branches.filter((branch) => branch.isActive).map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</AppSelect></label><label>Name<RequiredMark /><input name="name" defaultValue={value("name")} required minLength={2} maxLength={200} placeholder="Main warehouse" /></label><label>Address<input name="address" defaultValue={value("address")} maxLength={1000} placeholder="Kathmandu" /></label><label>Default<AppSelect name="isDefault" defaultValue={boolean("isDefault")}><option value="false">No</option><option value="true">Yes</option></AppSelect></label><label className="accounting-form__wide">Description<textarea name="description" rows={2} defaultValue={value("description")} maxLength={1000} placeholder="Optional warehouse description" /></label></>;
...
function MastersCatalogPage({ type, createPage, editId }: { type: string; createPage?: boolean; editId?: string }) {
  ...
  if (editId) {
    ...
    return <CatalogEditPage type={type as CatalogResource} item={item} categories={categories.data ?? []} contactGroups={contactGroups.data ?? []} branches={branches.data ?? []} />;
  }
  const simpleForm =
  ...
  if (type === "warehouses") return <Catalog<Warehouse> title="Warehouses" description="Maintain locations used for inventory operations." loading={warehouses.isLoading || branches.isLoading} error={warehouses.error ?? branches.error} create={warehouseCreate} columns={["Name", "Address", "Default"]} row={(x) => <><td>{x.name}</td><td>{x.address ?? "—"}</td><td>{x.isDefault ? "Yes" : "No"}</td></>} form={simpleForm(<><label>Branch<RequiredMark /><AppSelect name="branchId" defaultValue="" required><option value="">Select branch</option>{branches.data?.filter((branch) => branch.isActive).map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</AppSelect></label><label>Name<input name="name" placeholder="Main warehouse" required minLength={2} maxLength={200} /></label><label>Address<input name="address" placeholder="Kathmandu" maxLength={1000} /></label><label>Default<AppSelect name="isDefault" defaultValue="false"><option value="false">No</option><option value="true">Yes</option></AppSelect></label><label className="accounting-form__wide">Description<textarea name="description" rows={2} placeholder="Optional warehouse description" maxLength={1000} /></label></>)} masterType={type} createPage={createPage} />;
