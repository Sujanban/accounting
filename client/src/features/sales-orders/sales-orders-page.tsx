...
export function SalesOrdersPage() {
  ...
  return (
    <Flex direction="column" gap="5">
      <Header title="Sales orders" description="Planning documents only; sales orders do not affect inventory or accounting." action={<Button onClick={() => navigate("/sales-orders/new")}>Add sales order</Button>} />
      <Card size="3"><div className="accounting-filters"><label>Branch<AppSelect value={branchId} onChange={(event) => setBranchId(event.target.value)}><option value="">All branches</option>{branchFilter.data?.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</AppSelect></label><label>Status<AppSelect value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="DRAFT">Draft</option><option value="CONFIRMED">Confirmed</option><option value="CANCELLED">Cancelled</option><option value="CLOSED">Closed</option></AppSelect></label></div></Card>
      {changeStatus.error || closeOrder.error || convert.error ? <Text color="red" role="alert">{errorMessage(changeStatus.error || closeOrder.error || convert.error)}</Text> : null}
        ...
        <table className="accounting-table">
          ...
          <tbody>
            {matchingOrders.map((order) => {
              const pending = changeStatus.isPending || closeOrder.isPending || convert.isPending;
              const actions = order.status === "DRAFT" ? [
                { label: "Edit order", icon: <Pencil1Icon />, onSelect: () => navigate(`/sales-orders/${order.id}/edit`) },
...
export function SalesOrderDeliveryPage() {
  ...
  return <Flex direction="column" gap="5"><Header title="Create delivery note" description={`Record delivery for sales order ${order.orderNumber}.`} /><Card size="3"><form className="accounting-form" onSubmit={(event) => { event.preventDefault(); void (async () => { try { await delivery.mutateAsync(); await client.invalidateQueries({ queryKey: ["sales-orders"] }); navigate("/sales-orders", { replace: true }); } catch { /* mutation state is rendered below */ } })(); }}><label>Warehouse<AppSelect value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)} required><option value="">Select warehouse</option>{warehouses.data?.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</AppSelect></label><label>Delivery date (BS)<NepaliDatePicker value={fulfillmentDate} onChange={setFulfillmentDate} required ariaLabel="Choose delivery date in Bikram Sambat" /></label><div className="accounting-form__actions accounting-form__wide"><Button type="button" variant="outline" onClick={() => navigate("/sales-orders")}>Cancel</Button><Button type="submit" disabled={!warehouseId || !fulfillmentDate} loading={delivery.isPending}>Post delivery note</Button></div></form>{delivery.error ? <Text color="red" role="alert">{errorMessage(delivery.error)}</Text> : null}</Card></Flex>;
}
