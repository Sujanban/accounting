...
function AssetForm({ asset }: { asset?: FixedAsset }) {
  ...
  return <Flex direction="column" gap="5"><CrudPageHeader title={asset ? "Edit fixed asset" : "Add fixed asset"} description="Maintain acquisition values and depreciation settings." /><CrudPageState loading={branches.isLoading} error={branches.error} label="Loading fixed asset" description="Preparing the asset form…"><Card size="3"><form className="accounting-form" onSubmit={(event) => void submit(event)}><label>Branch<AppSelect value={branchId} onChange={(event) => setBranchId(event.target.value)} required><option value="">Select branch</option>{branches.data?.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</AppSelect></label><label>Asset code<input name="assetCode" defaultValue={asset?.assetCode} required /></label><label>Category<input name="category" defaultValue={asset?.category} required /></label><label>Purchase date<input name="purchaseDate" type="date" defaultValue={asset?.purchaseDate.slice(0, 10)} required /></label><label>Purchase value<input name="purchaseValue" type="number" min="0" step="any" defaultValue={asset?.purchaseValue} required /></label><label>Salvage value<input name="salvageValue" type="number" min="0" step="any" defaultValue={asset?.salvageValue ?? 0} required /></label><label>Useful life (months)<input name="usefulLifeMonths" type="number" min="1" defaultValue={asset?.usefulLifeMonths} required /></label><label>Method<AppSelect name="depreciationMethod" defaultValue={asset?.depreciationMethod ?? "STRAIGHT_LINE"}><option value="STRAIGHT_LINE">Straight line</option><option value="WRITTEN_DOWN_VALUE">Written down value</option></AppSelect></label><div className="accounting-form__actions accounting-form__wide"><Button type="button" variant="outline" onClick={() => navigate("/fixed-assets")}>Cancel</Button><Button type="submit" disabled={!branchId} loading={create.isPending || update.isPending}>{asset ? "Save asset" : "Add asset"}</Button></div></form>{create.error || update.error ? <Text color="red" role="alert">{requestMessage(create.error || update.error)}</Text> : null}</Card></CrudPageState></Flex>;
}
...
function DepreciationForm({ asset }: { asset: FixedAsset }) {
  ...
  return <Flex direction="column" gap="5"><CrudPageHeader title="Prepare depreciation journal" description={`Create a reviewable journal draft for ${asset.assetCode}.`} /><CrudPageState loading={ledgers.isLoading} error={ledgers.error} label="Loading ledgers" description="Preparing depreciation accounts…"><Card size="3"><form className="accounting-form" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}><label>Depreciation period<AppSelect value={periodMonth} onChange={(event) => setPeriodMonth(event.target.value)}>{Array.from({ length: asset.usefulLifeMonths }, (_, index) => <option key={index + 1} value={index + 1}>Month {index + 1}</option>)}</AppSelect></label><label>Journal date<input type="date" value={transactionDate} onChange={(event) => setTransactionDate(event.target.value)} required /></label><label>Expense ledger<AppSelect value={expenseLedgerId} onChange={(event) => setExpenseLedgerId(event.target.value)} required><option value="">Select ledger</option>{ledgers.data?.map((ledger) => <option key={ledger.id} value={ledger.id}>{ledger.name}</option>)}</AppSelect></label><label>Accumulated depreciation ledger<AppSelect value={accumulatedDepreciationLedgerId} onChange={(event) => setAccumulatedDepreciationLedgerId(event.target.value)} required><option value="">Select ledger</option>{ledgers.data?.map((ledger) => <option key={ledger.id} value={ledger.id}>{ledger.name}</option>)}</AppSelect></label><div className="accounting-form__actions accounting-form__wide"><Button type="button" variant="outline" onClick={() => navigate("/fixed-assets")}>Cancel</Button><Button type="submit" disabled={!expenseLedgerId || !accumulatedDepreciationLedgerId || expenseLedgerId === accumulatedDepreciationLedgerId} loading={create.isPending}>Create journal draft</Button></div></form>{create.error ? <Text color="red" role="alert">{requestMessage(create.error)}</Text> : null}</Card></CrudPageState></Flex>;
}
...
function DisposalForm({ asset }: { asset: FixedAsset }) {
  ...
  return <Flex direction="column" gap="5"><CrudPageHeader title="Prepare disposal journal" description={`Create a reviewable disposal draft for ${asset.assetCode}.`} /><CrudPageState loading={ledgers.isLoading} error={ledgers.error} label="Loading ledgers" description="Preparing disposal accounts…"><Card size="3"><form className="accounting-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); create.mutate({ proceeds: Number(form.get("proceeds")), accumulatedDepreciation: Number(form.get("accumulatedDepreciation")) }); }}><label>Journal date<input type="date" value={transactionDate} onChange={(event) => setTransactionDate(event.target.value)} required /></label><label>Proceeds<input name="proceeds" type="number" min="0" step="any" defaultValue="0" required /></label><label>Accumulated depreciation<input name="accumulatedDepreciation" type="number" min="0" max={asset.purchaseValue - asset.salvageValue} step="any" defaultValue="0" required /></label>{([['assetCostLedgerId', 'Asset cost ledger'], ['accumulatedDepreciationLedgerId', 'Accumulated depreciation ledger'], ['proceedsLedgerId', 'Proceeds ledger'], ['gainLossLedgerId', 'Gain/loss ledger']] as const).map(([field, label]) => <label key={field}>{label}<AppSelect value={ids[field]} onChange={(event) => setIds((current) => ({ ...current, [field]: event.target.value }))} required><option value="">Select ledger</option>{ledgers.data?.map((ledger) => <option key={ledger.id} value={ledger.id}>{ledger.name}</option>)}</AppSelect></label>)}<div className="accounting-form__actions accounting-form__wide"><Button type="button" variant="outline" onClick={() => navigate("/fixed-assets")}>Cancel</Button><Button type="submit" disabled={Object.values(ids).some((id) => !id)} loading={create.isPending}>Create disposal draft</Button></div></form>{create.error ? <Text color="red" role="alert">{requestMessage(create.error)}</Text> : null}</Card></CrudPageState></Flex>;
}
