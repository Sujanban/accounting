...
function VatRegisterReport({ type }: { type: "sales" | "purchase" }) {
  ...
  return <><ReportFiltersForm filters={filters} onApply={setFilters} />{report.isLoading ? <LoadingScreen fullScreen={false} label={`Loading ${title}`} description="Preparing VAT transactions…" /> : report.isError ? <Text color="red" role="alert">{errorMessage(report.error)}</Text> : report.data ? <ReportFrame printTitle={title}><div className="report-summary"><span>Taxable amount<strong>{money.format(report.data.totals.taxableAmount)}</strong></span><span>VAT<strong>{money.format(report.data.totals.vatAmount)}</strong></span><span>Total<strong>{money.format(report.data.totals.totalAmount)}</strong></span></div><table className="accounting-table"><thead><tr><th>Date</th><th>Voucher</th><th>Tax invoice</th><th>Party</th><th>PAN</th><th>Taxable</th><th>VAT</th><th>Total</th></tr></thead><tbody>{report.data.items.map((item) => <tr key={item.id}><td>{date(item.transactionDate)}</td><td><Link to={`/vouchers/transactions/${item.id}`}>{item.voucherNumber}</Link></td><td>{item.taxInvoiceNumber || "—"}</td><td>{item.partyName || "—"}</td><td>{item.panNumber || "—"}</td><td>{money.format(item.taxableAmount)}</td><td>{money.format(item.vatAmount)}</td><td>{money.format(item.totalAmount)}</td></tr>)}</tbody></table><ReportPagination meta={report.data.meta} onPageChange={(page) => setFilters({ ...filters, page })} />{report.data.items.length === 0 ? <Text as="p" color="gray" className="accounting-empty">No taxable {type} vouchers match these filters.</Text> : null}</ReportFrame> : null}</>;
}
