...
function AuthLayout({ children }: { children: React.ReactNode }) {
  return <main className="auth-layout grid min-h-screen place-items-center p-5"><div className="w-full max-w-md"><Link to="/" className="auth-brand mb-8"><span className="brand-mark">L</span>Ledgerly</Link>{children}</div></main>;
}
...
export function LoginPage() {
  ...
  return <AuthLayout><Card className="auth-card" size="4"><form onSubmit={submit} noValidate><Flex direction="column" gap="4"><div><Heading size="6">Sign in</Heading><Text as="p" color="gray" mt="2">Access your accounting workspace.</Text></div>{registeredEmail ? <Text color="green" size="2" role="status">Account created. Sign in to continue.</Text> : null}<FormTextField label="Email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required startAdornment={<PersonIcon />} error={error?.includes("email") ? error : undefined} /><FormTextField label="Password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required error={error && !error.includes("email") ? error : undefined} startAdornment={<LockClosedIcon />} showPasswordToggle /><Button type="submit" size="3" loading={isPending}>Sign in</Button><Text align="center" size="2" color="gray">New here? <Link className="auth-link" to="/register">Create an account</Link></Text></Flex></form></Card></AuthLayout>;
}
...
export function RegisterPage() {
  ...
  return <AuthLayout><Card className="auth-card" size="4"><form onSubmit={submit} noValidate><Flex direction="column" gap="4"><div><Heading size="6">Create account</Heading><Text as="p" color="gray" mt="2">Start by creating your secure workspace account.</Text></div><FormTextField label="Name" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" required minLength={2} error={fieldErrors.name} startAdornment={<PersonIcon />} /><FormTextField label="Email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required error={fieldErrors.email} startAdornment={<PersonIcon />} /><FormTextField label="Password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" required minLength={8} error={fieldErrors.password} description="Use 8+ characters with uppercase, lowercase, number, and special character." startAdornment={<LockClosedIcon />} showPasswordToggle /><FormTextField label="Confirm password" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" required minLength={8} error={fieldErrors.confirmPassword} startAdornment={<LockClosedIcon />} showPasswordToggle />{error ? <Text color="red" size="2" role="alert">{error}</Text> : null}<Button type="submit" size="3" loading={registerMutation.isPending}>Create account</Button><Text align="center" size="2" color="gray">Already have an account? <Link className="auth-link" to="/login">Sign in</Link></Text></Flex></form></Card></AuthLayout>;
}
