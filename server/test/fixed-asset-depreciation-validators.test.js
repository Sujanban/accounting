...
test("accepts a manual depreciation journal draft", () => {
  assert.deepEqual(validateDepreciationDraft({ periodMonth: 1, transactionDate: "2026-07-29", expenseLedgerId, accumulatedDepreciationLedgerId }), []);
});

test("rejects unsafe and malformed depreciation journal draft input", () => {
  const errors = validateDepreciationDraft({ periodMonth: 0, transactionDate: "2026-02-30", expenseLedgerId, accumulatedDepreciationLedgerId: expenseLedgerId, status: "POSTED" });
