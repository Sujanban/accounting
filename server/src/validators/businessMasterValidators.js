...
module.exports = {
  ...
  validateContactGroup: (body) => { const e = []; if (!body.name || body.name.trim().length < 2) e.push({ field: "name", message: "Contact group name is required." }); if (body.parentId && !mongoose.isObjectIdOrHexString(body.parentId)) e.push({ field: "parentId", message: "Parent group must be a valid identifier." }); return e; },
  validateWarehouse: (body) => { const e = []; if (!body.warehouseCode || !/^[A-Za-z0-9_-]{2,30}$/.test(body.warehouseCode.trim())) e.push({ field: "warehouseCode", message: "Warehouse code is invalid." }); if (!body.name || body.name.trim().length < 2) e.push({ field: "name", message: "Warehouse name is required." }); return e; },
  validatePriceList: (body) => { const e = []; if (!body.name || body.name.trim().length < 2) e.push({ field: "name", message: "Price list name is required." }); if (body.currency && !/^[A-Za-z]{3}$/.test(body.currency.trim())) e.push({ field: "currency", message: "Currency must be a 3-letter code." }); return e; },
