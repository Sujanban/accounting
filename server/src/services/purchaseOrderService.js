...
const map = (order) => ({ id: order._id, branchId: order.branchId, contactId: order.contactId, orderNumber: order.orderNumber, orderDate: order.orderDate, items: order.items, notes: order.notes, status: order.status, createdAt: order.createdAt, updatedAt: order.updatedAt });
...
async function list(companyId, query) { const page = Math.max(1, Number(query.page) || 1); const limit = Math.min(100, Math.max(1, Number(query.limit) || 20)); const filters = { companyId }; if (["DRAFT", "CONFIRMED", "CANCELLED"].includes(query.status)) filters.status = query.status; const [items, total] = await Promise.all([PurchaseOrder.find(filters).sort({ orderDate: -1, _id: -1 }).skip((page - 1) * limit).limit(limit).lean(), PurchaseOrder.countDocuments(filters)]); return { items: items.map(map), meta: { page, limit, total, totalPages: Math.ceil(total / limit), hasNextPage: page * limit < total } }; }
