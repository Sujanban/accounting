const mongoose = require("mongoose"); const { PurchaseOrder } = require("../models/PurchaseOrder"); const { Branch } = require("../models/Branch"); const { Contact } = require("../models/Contact"); const { Product } = require("../models/Product"); const { ApiError } = require("../utils/apiError");
...
async function list(companyId, query) { const page = Math.max(1, Number(query.page) || 1); const limit = Math.min(100, Math.max(1, Number(query.limit) || 20)); const filters = { companyId }; if (["DRAFT", "CONFIRMED", "CANCELLED", "CLOSED"].includes(query.status)) filters.status = query.status; if (query.branchId !== undefined) { if (!mongoose.isObjectIdOrHexString(query.branchId)) throw new ApiError(422, "Branch must be a valid identifier."); filters.branchId = query.branchId; } const [items, total] = await Promise.all([PurchaseOrder.find(filters).sort({ orderDate: -1, _id: -1 }).skip((page - 1) * limit).limit(limit).lean(), PurchaseOrder.countDocuments(filters)]); return { items: items.map(map), meta: { page, limit, total, totalPages: Math.ceil(total / limit), hasNextPage: page * limit < total } }; }
